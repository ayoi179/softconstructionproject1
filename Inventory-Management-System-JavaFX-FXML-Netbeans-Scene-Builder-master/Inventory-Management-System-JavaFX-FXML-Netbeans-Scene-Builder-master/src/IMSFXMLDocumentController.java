...
// Setting onMousePress event handler for detecting click on table element for guture modify and delete action
        ...
        // When a part is selected but it is associated with a product
            ...
            if (result.get() == ButtonType.OK) {
                try {
                    ...
                    products.stream().filter((p) -> (partSelected.associatedPartID == p.getProductID())).forEachOrdered((p) -> {
                        productSelected = p;
                    });
                    ...
                    parts.stream().filter((p) -> (productSelected.getProductID() == p.getAssociatedPartID())).forEachOrdered((p) -> {
    ...
    void deleteProductSelected(ActionEvent event) {
        ...
        if (productSelected != null) {
            ...
            warningText = parts.stream().filter((p) -> (p.getAssociatedPartID() == productSelected.getProductID())).map((p) -> "\nProduct ID : "+p.getPartsID()).reduce(warningText, String::concat);
